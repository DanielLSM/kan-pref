...
class PreferenceCallback(BaseCallback):
    ...
    def train_reward_model(self, training_epochs):
        ...
        self.hc.train_dataset_with_critical_points(my_dataloader, {}, epochs_override=training_epochs)
...
class UpdateRewardFunctionCriticalPoint(PreferenceCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        print("location: ")
        print(os.getcwd())
        self.hc.save_buffers(os.getcwd(), "cheetah")
        if wandb.run is not None:
            wandb.save(f'{os.getcwd()}*')
...
import os
class Moderate(PreferenceCallback):
    """
    Callback to periodically gather query feedback and updates the reward model.
    """
    def __init__(self, **kwargs):
        super(Moderate, self).__init__(**kwargs)
        print("Moderate")

    def _on_training_start(self) -> None:
        print("Performing initial training of reward model")

        if self.n_initial_queries > 0:
            # Collect segments
            trajectories, critical_points = self.generate_segments(queries_multiplier=5, n_past_batches=1)
    ...
    def _on_step(self) -> bool:
        ...
        if self.hc.pairs_size < self.max_queries and self.n_queries > 0:
            # Collect segments
            trajectories, critical_points = self.generate_segments(queries_multiplier=5)
    ...
    def train_reward_model(self, epoch):
        my_dataloader, my_dataloader_cost = self.prepare_training_data()
        self.hc.train_dataset_with_critical_points(my_dataloader, my_dataloader_cost, epoch)
    ...
    def prepare_training_data(self):
        ...
        o1, o2, prefs, critical_points = self.hc.get_all_preference_pairs_cost()
        tensor_o1 = torch.Tensor(o1)
        tensor_o2 = torch.Tensor(o2)
        tensor_prefs = torch.Tensor(prefs)
        tensor_critical_points = torch.Tensor(critical_points)
        my_dataset = TensorDataset(tensor_o1, tensor_o2, tensor_prefs, tensor_critical_points)
        my_dataloader_cost = DataLoader(my_dataset, batch_size=self.hc.batch_size, shuffle=True)
        return my_dataloader, my_dataloader_cost
