...
def env_step(env, action):
    ...
    if len(env_obj) == 5:
        next_state, reward, done, truncated, info = env_obj
    else:
        next_state, reward, done, info = env_obj
        truncated = None
...
class TruncatedOnSuccessWrapper(gym.Wrapper):
    ...
    def step(self, action) -> GymStepReturn:
        obs, reward, terminated, truncated, info = env_step(self.env, action)
...
class ActionSmoothingWrapper(gym.Wrapper):
    ...
    def step(self, action) -> GymStepReturn:
        ...
        return env_step(self.env, self.smoothed_action)
...
class HistoryWrapperObsDict(gym.Wrapper):
    ...
    def step(self, action) -> Tuple[Dict[str, np.ndarray], SupportsFloat, bool, bool, Dict]:
        obs_dict, reward, terminated, truncated, info = env_step(self.env, action)
        obs = obs_dict["observation"]
        last_ax_size = obs.shape[-1]
...
class HumanReward(gym.Wrapper):
    ...
    def __init__(self, env, hc, n_envs=1):
        ...
        self.crashes = collections.deque(maxlen=20)
        self.running_average = collections.deque(maxlen=10)
        self.episode_reward = 0
        self.metrics_saved = []
        if wandb.run is not None:
            wandb.define_metric("reward_metric/step")
            wandb.define_metric("reward_metric/*", step_metric="reward_metric/step")

    @torch.no_grad()
    def step(self, action):
        observation = self.get_human_reward_observation(action)

        next_state, reward, done, truncated, info = env_step(self.env, action)
        ...
        if done or truncated:
            if wandb.run is not None:
                wandb.log({"reward_metric/true_reward": self.episode_true_reward,
                           "reward_metric/human_reward": self.episode_reward_human,
                           "reward_metric/step": self.t})
            self.episode_reward_human = 0
            self.episode_true_reward = 0

        # TODO this is RNN code
        if done and False:
            self.human_model.reward_model.reset_states()
    ...
    def get_human_reward_observation(self, action):
        ...
        is_tuple = isinstance(self.current_state, (tuple, Tuple))
        current_state = self.current_state[0] if is_tuple else self.current_state
        if len(action_tensor.shape) != 0:
            state_tensor = torch.tensor(current_state)
            ...
            observation = torch.cat([torch.tensor(current_state), action_tensor])
...
class MetaworldStopping(gym.Wrapper):
    """
    Environment wrapper to replace the env reward function with a human based reward function.
    In addition, it logs both the env and human reward per episode.
    """
    ...
    def __init__(self, env):
        super().__init__(env)
        if wandb.run is not None:
            wandb.define_metric("rollout/success", summary="mean")

    def step(self, action):
        next_state, reward, done, truncated, info = env_step(self.env, action)
        if wandb.run is not None and (done or truncated):
            success = info["success"]
            print(success)
            wandb.log({"rollout/success": success})
