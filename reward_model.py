...
class RewardModel:
    def __init__(self, ds, da, 
        ...
        # Load pretrained function
        if pretrained_model:
        self.load2(model_path, model_name)
        """

        if self.use_l2_regularization:
            self.original_weights = []
            for member in range(self.de):
                initial_weights = {name: param.clone() for name, param in self.ensemble[member].named_parameters()}
                self.original_weights.append(initial_weights)
        ...
        if self.use_kl_regularization:
            ...
            for member in range(self.de):
                self.original_models[member].load_state_dict(
                    torch.load('%s/%s_%s.pt' % ("/root/code/rl_zoo", model_name, member)), strict=False
                )
    ...
    def train_reward(self):
        ...
        for epoch in range(num_epochs):
            ...
            for member in range(self.de):
                ...
                # get logits
                old_r_hat1 = self.r_hat_member(sa_t_1, member=member)
                old_r_hat2 = self.r_hat_member(sa_t_2, member=member)
                r_hat1 = old_r_hat1.sum(axis=1)
                r_hat2 = old_r_hat2.sum(axis=1)
                r_hat = torch.cat([r_hat1, r_hat2], axis=-1)
